 ...
 */
final class FooServer implements RequestHandlerInterface
{
    const PATH_PREFIX = '/twirp/foo.Foo/';
    ...
    public function handle(ServerRequestInterface $req): ResponseInterface
    {
        ...
        $ctx = Context::withPackageName($ctx, 'foo');
        $ctx = Context::withServiceName($ctx, 'Foo');
        ...
        switch ($req->getUri()->getPath()) {
            case '/twirp/foo.Foo/Bar':
                return $this->handleBar($ctx, $req);
    ...
    private function handleBar(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        ...
        switch (trim(strtolower(substr($header, 0, $i)))) {
            case 'application/json':
                $resp = $this->handleBarJson($ctx, $req);
                break;

            case 'application/protobuf':
                $resp = $this->handleBarProtobuf($ctx, $req);
                break;
    ...
    private function handleBarJson(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        $ctx = Context::withMethodName($ctx, 'Bar');
        ...
        try {
            ...
            $out = $this->svc->Bar($ctx, $in);

            if ($out === null) {
                return $this->writeError($ctx, TwirpError::newError(ErrorCode::Internal, 'received a null response while calling Bar. null responses are not supported'));
            }
    ...
    private function handleBarProtobuf(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        $ctx = Context::withMethodName($ctx, 'Bar');
